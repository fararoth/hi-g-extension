...
import AppLeftPopup from "app/components/blocks/AppLeftPopup";
...
const AddAccountModal = memo(() => {
  ...
  return (
    <Dialog.Root open={accModalOpened} onOpenChange={handleOpenChange} modal>
      <Dialog.Portal>
        <Dialog.Overlay
          className={classNames("fixed inset-0 z-20", "create-account-bg")}
        />
        <Dialog.Content
          className={classNames(
            "fixed z-20",
            "w-full md:min-w-[40rem]",
        ...
        >
          <div className="flex">
            ...
            <div className="w-2/3">
              {/* Existing content and components here */}
              <OnMount handle={handleContentMount} />
              {/* <WigwamIcon className="w-16 h-auto absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/4 z-30" /> */}
              <OverflowProvider>
                {(ref) => (
                  <ScrollAreaContainer
                    ref={ref}
                  ...
                  >
                    {isInitial && (
                      <div className="absolute inset-0 z-[-5] rounded-[2.5rem] overflow-hidden bg-brand-dark/10 backdrop-blur-[30px] IS_FIREFOX && 'brandbg-large-modal'" />
                    )}
                    <BackButton
